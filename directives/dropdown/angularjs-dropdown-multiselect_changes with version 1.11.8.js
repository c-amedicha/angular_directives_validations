/*
 * angularjs-dropdown-multiselect - v1.11.8
 * https://github.com/dotansimha/angularjs-dropdown-multiselect
 * Copyright under MIT Open Source License (https://github.com/dotansimha/angularjs-dropdown-multiselect/blob/master/LICENSE)
 * Implemented By : Anusha Medicharla (Department Of Corrections Pennsylvania) - 06/17/2017
 * Changes:
 *  Change#1 - Added "ng-model-options" attribute to have the input box update on change by default, not affected by the parent's ng-model-options.
 *  Change#2 - Added a watch to initialize the "selectedModel"  attribute's assigned value to an object or array, depending on whether it is a single select   
 *              dropdown or a multiselect dropdown.
 */
...
directiveModule.directive('ngDropdownMultiselect', ['$filter', '$document', '$compile', '$parse', function($filter, $document, $compile, $parse) {
	return {
		...
		template: function(element, attrs) {
            ...
            //Change#1 Start- Added "ng-model-options" attribute to have the input box update on change by default, not affected by the parent's ng-model-options.
			template += '<li ng-if="settings.enableSearch"><div class="dropdown-header"><input type="text" class="form-control searchField" ng-model-options="{updateOn: \'default\' ,allowInvalid: true , debounce: 0}" ng-keydown="keyDownSearchDefault($event); keyDownSearch($event, input.searchFilter);" ng-style="{width: \'100%\'}" ng-model="input.searchFilter" placeholder="{{texts.searchPlaceholder}}" /></li>';
            //Change#1 End
		...
		link: function($scope, $element, $attrs) {
            ...
            //Change#2 Start - Added a watch to initialize the "selectedModel"  attribute's assigned value to an object or array, depending on whether it is a single select dropdown or a multiselect dropdown.
            $scope.$watch('singleSelection', function(newValue) {
				if (newValue==true) {
					$scope.selectedModel = {};
				} else {
					$scope.selectedModel = [];
