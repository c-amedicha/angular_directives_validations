 ...
 * Copyright under MIT Open Source License (https://github.com/jonsamwell/angular-auto-validate/blob/master/LICENSE)
 * Implemented By : Anusha Medicharla (Department Of Corrections Pennsylvania) - 06/17/2017
 * Changes:
 *  Change#1,Change#3,Change#6,Change#7 - Changed from $log.error to $log.warn to avoid console errors for not adding a form-group class.
 *  Change#2,Change#5 - Added validations for ui-tinymce component. This will add the css style in case of a validation error.
 *  Change#4 - Added code to check for a parent DIV to place the validation error message after.
 *  Change#8 - Added types div and button to elementTypesToValidate for the dropdown and datepicker.
 */
  ...
  var reset = function (el) {
    ...
    makeValid = function (el) {
      ...
      if (frmGroupEl) {
        ...
        //Change#1 Start- Changed from $log.error to $log.warn to avoid console errors for not adding a form-group class.
            $log.warn('Angular-auto-validate: invalid bs3 form structure elements must be wrapped by a form-group class');
        //Change#1 End
      }
    },
    ...
    makeInvalid = function (el, errorMsg) {
      //Change#2 Start - Added validations for ui-tinymce component. This will add the css style in case of a validation error .
      if(el[0].id.indexOf("uiTinymce") !== -1){
          el.parent().find('label').addClass("captorValidationErrorLabel");
          el.parent().addClass('captorTinyMceError');
      }
      //Change#2 End
      var frmGroupEl = findFormGroupElement(el),
      ...
      if (frmGroupEl) {
        ...
        //Change#3 Start - Changed from $log.error to $log.warn to avoid console errors for not adding a form-group class.
            $log.warn('Angular-auto-validate: invalid bs3 form structure elements must be wrapped by a form-group class');
        //Change#3 End
      }
    },
    ...
    getCorrectElementToPlaceErrorElementAfter = function (el) {
      ...
      //Change#4 Start - Added code to check for a parent DIV to place the validation error message after.
      if ((elType === 'checkbox' || elType === 'radio') && (el.parent()[0].nodeName.toLowerCase() === 'label' || el.parent()[0].nodeName.toLowerCase() === 'div')) {
        correctEl = el.parent();
      }
     //Change#4 End
      return correctEl;
    ...
    makeDefault = function (el) {
      ...
      if (frmGroupEl) {
        reset(frmGroupEl);
        //Change#5 Start- Added validations for ui-tinymce component. This will add the css style in case of a validation error.
        if(el[0].id.indexOf("uiTinymce") !== -1){
            el.parent().find('label').removeClass("captorValidationErrorLabel");
            el.parent().removeClass('captorTinyMceError');
        }
        //Change#5 End
      } else {
        //Change#6 Start- Changed from $log.error to $log.warn to avoid console errors for not adding a form-group class.
            $log.warn('Angular-auto-validate: invalid bs3 form structure elements must be wrapped by a form-group class');
        //Change#6 End
      }
    },
    ...
    waitForAsyncValidators = function (el) {
      ...
      if (frmGroupEl) {
        ...
        //Change#7 Start- Changed from $log.error to $log.warn to avoid console errors for not adding a form-group class.
            $log.warn('Angular-auto-validate: invalid bs3 form structure elements must be wrapped by a form-group class');
        //Change#7 End
      }
    };
...
function ValidationManagerFn(validator, elementUtils, $anchorScroll) {
  //Change#8 Start- Added types div and button to elementTypesToValidate for the dropdown and datepicker.
  var elementTypesToValidate = ['input', 'textarea', 'select', 'form', 'div','button'],
  //Change#8 End
